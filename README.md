...
```
Error: FileNotFoundError - 'models/rag_lyrics_model.h5' not found
```
**A:** Ensure all model files exist in the `models/` directory:
- `rag_lyrics_model.h5`
- `tokenizer.pickle`
...
```
Error: 'ArianaGrande.csv' not found
```
**A:** Place the CSV file in the project root directory and ensure it has a `Lyric` column.
...
**Q: Sequence length mismatch**
```
Error: Shape mismatch - expected (100,) got (X,)
```
**A:** Update `max_sequence_length` in `main.py` to match the sequence length used during model training.

**Q: Port already in use**
```
Error: Address already in use
```
**A:** Change the port in command:
```bash
...
- Dataset: Ariana Grande lyrics
- Framework: [Streamlit](https://streamlit.io/)
- ML Libraries: [TensorFlow](https://www.tensorflow.org/), [scikit-learn](https://scikit-learn.org/)
